...
import { API, showError, getRelativeTime, stripScriptTags } from '../../helpers';
...
const NoticeModal = ({
  ...
  const displayNotice = async () => {
    ...
    try {
      ...
      if (success) {
        if (data !== '') {
          const htmlNotice = stripScriptTags(marked.parse(data));
      ...
      <div className='max-h-[55vh] overflow-y-auto pr-2 card-content-scroll'>
        <Timeline mode='left'>
          {processedAnnouncements.map((item, idx) => {
            const htmlContent = stripScriptTags(marked.parse(item.content || ''));
            const htmlExtra = item.extra ? stripScriptTags(marked.parse(item.extra)) : '';
            return (
              <Timeline.Item
                key={idx}
                type={item.type}
